...
### MODEL WORK, STILL PARTIALLY IN PROGRESS ###
```{r}
...
# 2014
library(e1071)
data <- subset(businesses14_2, Inactive == 1)
...
pred <- predict(model, subset(data, select = -Inactive))
...
cm_original <- confusionMatrix(final$predict, test_data$Inactive)
cm_original
...
cm_under <- confusionMatrix(final_under$predict, test_data$Inactive)
cm_under
...
cm_over <- confusionMatrix(final_over$predict, test_data$Inactive)
cm_over
...
cm_rose <- confusionMatrix(final_rose$predict, test_data$Inactive)
cm_rose
...
cm_smote <- confusionMatrix(final_smote$predict, test_data$Inactive)
cm_smote
...
# RESULTS #
### Still in progress ###
As of right now I'm struggling with finding an algorithm to reliably predict inactive businesses in a given year using data about the location of that business from the year prior. It seems my models are able to accurately predict the active businesses fairly well, which account for about 90% of the overall dataset, however, my models are struggling to predict the Inactive businesses, which is the key piece I want to predict. 

At this stage, if there is feedback on my approach or potential other machine learning algorithms for imbalanced binary classification, that would be a significant help.
...
##### POTENTIAL RESOURCES #####
### IF THERE ARE OTHER RESOURCES FOR RARE/imbalanced CASE BINARY CLASSIFICATION THAT'D BE VERY HELPFUL.
## i'M SEEING One Class SVM, RANDOM FOREST, LOGIT MODELS, NEURAL NETWORK ARE ALL OPTIONS
---
##### USE THIS ONE #########
...
# USE THIS ONE https://www.analyticsvidhya.com/blog/2016/03/practical-guide-deal-imbalanced-classification-problems/
